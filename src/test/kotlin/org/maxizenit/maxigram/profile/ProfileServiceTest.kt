...
class ProfileServiceTest {
    ...
    @Test
    fun `saves a valid profile with interests`() {
        val profile = service.save(userId, "  Ann ", "Lee ", LocalDate.of(2000, 1, 1), listOf(1L))

        assertThat(profile.firstName).isEqualTo("Ann")
        assertThat(profile.lastName).isEqualTo("Lee")
        assertThat(profile.interests).extracting<String> { it.name }.containsExactly("Футбол")
        assertThat(profiles.findById(userId)).isNotNull()
    }

    @Test
    fun `rejects blank names`() {
        assertThatThrownBy { service.save(userId, "  ", "Lee", LocalDate.of(2000, 1, 1), emptyList()) }
            .isInstanceOf(InvalidProfileException::class.java)
    }

    @Test
    fun `rejects a birthdate that is not in the past`() {
        assertThatThrownBy { service.save(userId, "Ann", "Lee", LocalDate.of(2030, 1, 1), emptyList()) }
            .isInstanceOf(InvalidProfileException::class.java)
    }

    @Test
    fun `rejects unknown interest ids`() {
        assertThatThrownBy { service.save(userId, "Ann", "Lee", LocalDate.of(2000, 1, 1), listOf(99L)) }
            .isInstanceOf(InvalidProfileException::class.java)
    }

    @Test
    fun `find returns the saved profile with its interests`() {
        service.save(userId, "Ann", "Lee", LocalDate.of(2000, 1, 1), listOf(1L, 2L))
    ...
    private class FakeProfileRepository(private val catalogue: Map<Long, Interest>) : ProfileRepository {
        ...
        override fun upsert(userId: UUID, firstName: String, lastName: String, birthdate: LocalDate) {
            base[userId] = UserProfile(userId, firstName, lastName, birthdate, emptyList())
        }
